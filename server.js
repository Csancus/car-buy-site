...
// ── HTTP fetch via curl (avoids Node.js TLS fingerprint block) ───
function fetchUrl(targetUrl) {
  return new Promise((resolve, reject) => {
    ...
    execFile('curl', args, { maxBuffer: 30 * 1024 * 1024 }, (err, stdout, stderr) => {
      if (err) return reject(new Error(stderr || err.message));
      const sepIdx = stdout.lastIndexOf('\n__HTTP_STATUS__');
      const body = sepIdx >= 0 ? stdout.slice(0, sepIdx) : stdout;
      const status = sepIdx >= 0 ? parseInt(stdout.slice(sepIdx + 16), 10) : 0;
